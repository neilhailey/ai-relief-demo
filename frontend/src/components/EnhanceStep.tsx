...
export function EnhanceStep({ sessionId, originalUrl, subject, onComplete }: Props) {
  ...
  const features = [t.featureSharpen, t.featureClarity, t.featureDefinition]
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 28, padding: '36px 24px' }}>
      ...
      <div style={{ textAlign: 'center' }}>
        <h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 8 }}>{t.enhanceTitle}</h1>
        <p style={{ fontSize: 13, color: 'var(--text-dim)', maxWidth: 480 }}>{t.enhanceSubtitle}</p>
        {displaySubject && (
          <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 6, fontStyle: 'italic' }}>
            {t.detected} "{displaySubject}"
          </p>
        )}
      </div>
      ...
      >
        ...
        {enhancedUrl && (
          <div style={{ position: 'absolute', inset: 0, clipPath: `inset(0 ${100 - sliderPos}% 0 0)` }}>
            <img src={enhancedUrl} alt="Enhanced" draggable={false}
        ...
        {enhancedUrl && (
          <div style={{
            ...
            width: 2, background: 'rgba(255,255,255,.85)', pointerEvents: 'none',
          }}>
            <div style={{
              ...
              boxShadow: '0 2px 10px rgba(0,0,0,.4)', fontSize: 13, color: '#333', fontWeight: 700,
            }}>⟺</div>
          </div>
        )}
        ...
        {enhancedUrl && (
          <>
            <div style={{
              ...
              background: 'rgba(0,0,0,.55)', fontSize: 10, fontWeight: 600, color: '#fff', letterSpacing: '.06em',
            }}>{t.labelOriginal}</div>
            <div style={{
              position: 'absolute', top: 10, right: 10, padding: '3px 8px', borderRadius: 4,
              background: 'var(--accent)', fontSize: 10, fontWeight: 600, color: '#fff', letterSpacing: '.06em',
            }}>{t.labelEnhanced}</div>
          </>
        )}
        ...
        {status === 'enhancing' && (
          <div style={{
            ...
            <EnhanceSpinner />
            <div style={{ color: '#fff', fontSize: 14, fontWeight: 500 }}>{t.convertingStyle}</div>
            <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>{t.usuallyTime}</div>
          </div>
        )}
      </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {features.map(label => (
            <div key={label} style={{
              ...
              borderRadius: 8, fontSize: 11, fontWeight: 600, textAlign: 'center',
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
        {status === 'done' ? (
          <>
            ...
            >{t.useOriginal}</button>
            <button
              onClick={() => onComplete(enhancedUrl!, finalSubject, 1)}
              style={{
                padding: '12px 28px', background: 'var(--accent)', border: 'none',
                borderRadius: 'var(--radius)', color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer',
              }}
            >{t.useEnhanced}</button>
          </>
        ) : (
          <>
            <button
              ...
              style={{
                padding: '10px 20px', background: 'transparent',
                border: '1px solid var(--border)', borderRadius: 'var(--radius)',
                color: status === 'enhancing' ? 'var(--muted)' : 'var(--text-dim)', fontSize: 13,
                cursor: status === 'enhancing' ? 'default' : 'pointer',
              }}
            >{t.skipEnhancement}</button>
            <button
              ...
              style={{
                ...
                cursor: status === 'enhancing' ? 'default' : 'pointer', transition: 'background .2s',
              }}
            >
              {status === 'enhancing' ? <><SmallSpinner /> {t.enhancing}</> : t.enhanceBtn}
            </button>
          </>
        )}
      </div>
